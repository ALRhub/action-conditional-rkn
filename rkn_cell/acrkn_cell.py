...
#    @torch.jit.script_method
    ...
    def _build_transition_model(self) -> None:
        ...
        self._eye_matrix = nn.Parameter(torch.eye(self._lod).unsqueeze(0), requires_grad=False)

        self._tm_11_full = nn.Parameter(torch.zeros(self.c.num_basis, self._lod, self._lod, dtype=self._dtype))
        self._tm_12_full = \
            nn.Parameter(0.2 * torch.eye(self._lod, dtype=self._dtype).unsqueeze(0).repeat(self.c.num_basis, 1, 1))
        self._tm_21_full =\
            nn.Parameter(-0.2 * torch.eye(self._lod, dtype=self._dtype).unsqueeze(0).repeat(self.c.num_basis, 1, 1))
    ...
    def get_transition_model(self, post_mean: torch.Tensor) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        tm11_full, tm12_full, tm21_full, tm22_full = [x.unsqueeze(0) for x in self._transition_matrices_raw] #unsqueeze is more intuitive
